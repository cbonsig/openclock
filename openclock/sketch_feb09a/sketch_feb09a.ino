...
// 09 feb 2013 . first touch: enters menu and exits correctly
//                            and delays as desired when screen is touched
//               after clearning out of menu back to clock mode, and then entering menu mode
//                            again, it gets stuck in menu and never exits. why???
...
int lastTouchState = TOUCH_NONE;
boolean touchNew = false; // normally false, true for 1 cycle when touchscreen is pressed
...
boolean menuNew = false; // normally false, unless menu mode has just been activated
...
int detectTouch(){
  ...
  timer.enable(touchTimeout);   // start up the touch timer
  timer.restartTimer(touchTimeout);
...
void menuMode(){
  ...
  // decide what to do based on touch state, and get outta here
    ...
    // the menu state has not changed, and a new touch event has occured
  // so make menu and touch active flags are set
  // set the menuNew flag to true
  // and start the timer
  if (touchNew == true && timer.isEnabled(menuTimer) == true){
    menuActive = true;
    timer.restartTimer(menuTimer);   
  }
  else if (touchNew == true && timer.isEnabled(menuTimer) == false){
    menuActive = true;
    timer.enable(menuTimer);
    timer.restartTimer(menuTimer);      
  }

  // if there's no new touch, there's nothing to do here
  // just set menuActive flag, or cancel it, and leave
  if (touchNew == false && timer.isEnabled(menuTimer) == true){
    menuActive = true;
  }
  if (touchNew == false && timer.isEnabled(menuTimer) == false){
    menuActive = false;
    return;
  }

  // if we're still here, we must be in alarm mode, so set the message
  // accordingly, and decide what to change based on the area touched
